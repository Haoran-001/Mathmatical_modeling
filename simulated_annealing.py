...
# 子程序：定义优化问题的目标函数
# 目标函数定义为f(X) = 418.9829 * X_num - X * np.sin(np.sqrt(abs(X[i]))) f(X)得到的值越小越好
...
def OptimizationSSA(nVar,xMin,xMax,tInitial,tFinal,alfa,meanMarkov,scale):
    ...
    while tNow >= tFinal:           # 外循环，直到当前温度达到终止温度时结束
        ...
        recordFxBest.append(round(fxBest, 4))  # 当前内循环最佳解的目标函数值
        recordPBad.append(round(pBadAccept, 4))  # 当前内循环结束劣质解的接受概率
...
def main():
    ...
    X, Y = np.meshgrid(X, Y)
    R = X * np.sin(np.sqrt(abs(X))) + Y * np.sin(np.sqrt(abs(Y)))
    Z = 418.9829 * 2 - R
